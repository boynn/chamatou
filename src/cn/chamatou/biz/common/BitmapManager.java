...
public class BitmapManager {  
    ...
    public void queueJob(final String url, final ImageView imageView, final int width, final int height) {  
        ...
        pool.execute(new Runnable() {   
            public void run() {  
                ...
                message.obj = downloadBitmap((AppContext) imageView.getContext().getApplicationContext(),url, width, height);  
     ...
     * @param height
     */
    private Bitmap downloadBitmap(AppContext appContext,String url, int width, int height) {   
        Bitmap bitmap = null;
        try {
			//http加载图片
        	bitmap = ApiClient.getNetBitmap(appContext, url);
        	if(width > 0 && height > 0) {
				//指定显示图片的高宽
