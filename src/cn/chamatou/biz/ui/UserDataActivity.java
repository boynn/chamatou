 ...
 * 频道管理
 */
public class UserDataActivity extends BaseWithTitleActivity implements OnItemClickListener {
	public Context context;
	public static String TAG = "UserDataActivity";
	/** 用户栏目的GRIDVIEW */
	...
	/** 其它栏目列表 */
	List<ChannelItem> otherDataList = new ArrayList<ChannelItem>();
	/** 用户数据列表 */
	List<ChannelItem> userDataList = new ArrayList<ChannelItem>();
	...
	public void initView() {
		try {
			...
			userDataList = new ArrayList<ChannelItem>();
			otherDataList = new ArrayList<ChannelItem>();
			SharedPreferences sharedPreferences = new AccountTool(UserDataActivity.this).getPreferences();
			String menusStr = sharedPreferences.getString("datas", Const.datas.toString());
			JSONArray rows = new JSONArray(menusStr);
			HashSet<Integer> idSet = new HashSet<Integer>();
			for (int i = 0; i < rows.length(); i++) {
				Integer key = rows.getInt(i);
				idSet.add(key);
				if (Const.defaultUserDataMap.containsKey(key)) {
					userDataList.add(Const.defaultUserDataMap.get(key));
				}
			}
			for (ChannelItem channelItem : Const.defaultUserDataMap.values()) {
				if (!idSet.contains(channelItem.getId())) {
					otherDataList.add(channelItem);
				}
			}

			// 初始化数据
			userAdapter = new DragAdapter(this, userDataList);
			userGridView.setAdapter(userAdapter);
			otherAdapter = new OtherAdapter(this, otherDataList);
	...
	private class OnFinishClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			editor.putString("datas", rows.toString());
			editor.commit();
